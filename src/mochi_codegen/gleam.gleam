...
pub type GleamGenConfig {
  GleamGenConfig(
    ...
    /// Extra imports added to every resolver file (user-configured)
...
/// `types_import` is the full module path to import for types, e.g. "api/domain/user_types".
pub fn generate_resolvers(doc: SDLDocument, config: GleamGenConfig) -> String {
  generate_resolvers_with_types(doc, config, config.types_module)
}

pub fn generate_resolvers_with_types(
  doc: SDLDocument,
  ...
  let imports = extra_imports <> "import " <> types_import <> "\n\n"
